...
for fileloop in range(len(files)):
    ...
    M_x[fileloop] = int(dataset.EchoTime)

    print('M_dataset: %d' %(dataset.pixel_array[i,j]))
    print('M_x: %d' %(dataset.EchoTime))
...
# Function
def func_TE(x, a, b):
    return np.abs(      a * (np.exp( np.array(x) * (-1) / np.array(b)))        )
...
# 使用非线性最小二乘拟合
popt, pcov = curve_fit(func_TE, x, y, p0 = init_guess)
