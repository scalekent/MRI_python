...
for fileloop in range(len(files)):
    ...
    M_x[fileloop] = int(dataset.RepetitionTime)

    print('M_dataset: %d' %(dataset.pixel_array[i,j]))
    print('M_x: %d' %(dataset.RepetitionTime))
...
# Function
def func_TR(x, a, b):
    return np.abs(      a * (1 - np.exp( np.array(x) * (-1) / np.array(b)))        )
...
# 使用非线性最小二乘拟合
popt, pcov = curve_fit(func_TR, x, y, p0 = init_guess)
